# DeepNote AI x DeepBrain Integration — Project Plan
...
Transform DeepNote AI from a per-notebook tool into a cognitive operating layer for macOS through **bidirectional integration** with DeepBrain. DeepBrain handles OS-level services (file indexing, clipboard, Spotlight, ONNX embeddings, Ollama, cognitive cycles); DeepNote AI provides the rich notebook UI and content generation engine. Both systems expose local REST APIs and learn from each other, creating a compounding intelligence flywheel.

### Prerequisites

- DeepBrain app running (Tauri menu bar app)
- DeepBrain REST API on `localhost:19519`
- Ollama installed with models downloaded (optional but recommended)

---

## Phase 1: Foundation — DeepBrain Bridge Service

**Goal**: Establish the HTTP connection between DeepNote and DeepBrain. All subsequent phases build on this.

**Estimated effort**: 1-2 days

### 1.1 Create DeepBrain Bridge Service

**New file**: `src/main/services/deepbrain.ts` (~200 lines)

```typescript
// Core API client wrapping DeepBrain's REST endpoints
class DeepBrainService {
  ...
  // Configuration
  async getSettings(): Promise<DeepBrainSettings>
...
- All methods return `null` or empty arrays when DeepBrain is offline (graceful degradation)
...
**New file**: `src/main/ipc/deepbrain.ts` (~80 lines)

```typescript
// IPC channels:
// deepbrain:status     → DeepBrainStatus
// deepbrain:recall     → RecallItem[]
// deepbrain:search     → FileResult[]
// deepbrain:clipboard  → ClipboardEntry[]
// deepbrain:remember   → RememberResponse
// deepbrain:think      → ThinkResponse
// deepbrain:configure  → void (set port/token)
```
...
- `deepbrainStatus()`
- `deepbrainRecall(args)`
- `deepbrainSearch(args)`
- `deepbrainClipboard(args)`
- `deepbrainRemember(args)`
- `deepbrainThink(args)`
- `deepbrainConfigure(args)`

### 1.5 Add Connection Settings

**Modified file**: `src/renderer/src/components/common/SettingsModal.tsx`

Add a "DeepBrain" section:
...
- [ ] DeepBrain status shows in Settings when running
- [ ] Graceful "Not connected" when DeepBrain is off
...
**Goal**: Expose DeepNote's knowledge as a local REST API so DeepBrain, shell scripts, Raycast, and other tools can query notebooks, sources, chat history, and generated content.
...
POST /api/remember
  ...
  (Saves a chat message — allows DeepBrain to inject knowledge into notebooks)
...
Allow requests from DeepBrain's Tauri webview and local tools:
...
Add "DeepNote API" section under the DeepBrain section:
...
### 2.7 DeepBrain Configuration

DeepBrain needs to know DeepNote's API URL. Two approaches:

**Option A**: Add to DeepBrain's settings manually (user configures `deepnote_api_url`)
**Option B**: DeepNote registers itself with DeepBrain on startup via `POST /api/remember`:

```typescript
// On startup, tell DeepBrain about DeepNote
...
Recommended: **Both** — register on startup AND document in DeepBrain's config.
...
- [ ] DeepBrain can query DeepNote during cognitive cycles

### What This Enables

| Consumer | Use Case |
|----------|----------|
| **DeepBrain** | Polls `/api/notebooks` + `/api/search` during cognitive cycles to incorporate notebook knowledge |
...
**Goal**: Chat responses draw on DeepBrain's memories and file index in addition to notebook-specific RAG.

**Estimated effort**: 1 day

### 3.1 Inject DeepBrain Context into Chat
...
// NEW: DeepBrain system-wide context
let systemContext = ''
if (deepbrainService.isAvailable()) {
  const recalled = await deepbrainService.recall(args.message, 5)
...
```typescript
// Store key interaction in DeepBrain as episodic memory
deepbrainService.remember(
...
### 3.3 Inject DeepBrain Context into Voice

**Modified file**: `src/main/services/voiceSession.ts`

Add DeepBrain recall results to the voice session's system instruction, so the voice AI has system-wide context.

### Verification

- [ ] Chat responses reference system-wide memories when relevant
- [ ] Voice AI mentions system-wide context
- [ ] Chat interactions are stored as DeepBrain memories
- [ ] Works normally when DeepBrain is offline
...
Features:
- Unified search across DeepBrain files, memories, and Spotlight
...
**Modified file**: `src/main/ipc/deepbrain.ts`

Note: Spotlight search is a Tauri IPC command, not a REST endpoint. Two approaches:

**Option A**: Add a REST endpoint to DeepBrain for Spotlight (requires modifying DeepBrain)
**Option B**: Use macOS `mdfind` command directly from DeepNote's main process

Recommended: **Option B** (no DeepBrain changes needed)
...
Add "System Search" icon/link (only shown when DeepBrain is connected).
...
- [ ] Panel hides gracefully when DeepBrain is offline
...
```typescript
if (deepbrainAvailable) {
  const clipboard = await deepbrainService.getClipboardHistory(3)
...
- [ ] Works without DeepBrain (chip just doesn't appear)

---

## Phase 6: Auto-Remember and Learn

**Goal**: DeepNote automatically stores valuable content in DeepBrain's long-term memory.
...
After any studio content is generated, store a summary in DeepBrain:
...
- [ ] Studio generations appear in DeepBrain memories
- [ ] Source ingestions create DeepBrain memories
...
| Embeddings | ONNX (via DeepBrain) | Gemini |
...
**Goal**: DeepNote proactively suggests related content from DeepBrain when relevant.
...
When a chat response mentions topics that match DeepBrain memories, show a subtle "Related from your system" panel:
...
When opening a notebook, check DeepBrain for recently indexed files that are semantically related to the notebook's topic. Suggest them as potential sources.

### 8.3 Cross-Notebook Recommendations

Use DeepBrain's memory to suggest related content from other notebooks:
...
|------|-------|-------|---------|
| `src/main/services/deepbrain.ts` | 1 | ~200 | HTTP client for DeepBrain API |
| `src/main/ipc/deepbrain.ts` | 1 | ~80 | IPC handlers for DeepBrain |
...
| File | Phases | Changes |
|------|--------|---------|
| `src/shared/types/index.ts` | 1 | DeepBrain type definitions |
| `src/shared/types/ipc.ts` | 1 | New IPC channel definitions |
| `src/preload/index.ts` | 1 | 7 new API bridge methods |
| `src/main/index.ts` | 1, 2 | Register DeepBrain handlers, start DeepNote API server |
| `src/renderer/src/components/common/SettingsModal.tsx` | 1, 2, 7 | DeepBrain + DeepNote API + Ollama settings |
| `src/main/ipc/chat.ts` | 3, 6 | Inject DeepBrain context, auto-remember |
...
|------|--------|------------|
| DeepBrain not running | No system-wide features | Graceful degradation — all DeepBrain features are optional |
...
| Performance impact of dual search | Slower chat responses | 2s timeout on DeepBrain calls; parallel with RAG |
| DeepBrain API changes | Breaking integration | Version check on `/api/status`; pin to known API contract |
...
| Metric | Target |
|--------|--------|
| Chat response time with DeepBrain | < 5s (including dual context retrieval) |
...
| Graceful degradation | 100% — DeepNote works identically when DeepBrain is offline |
| Memory growth rate | ~10-50 memories/day with normal usage |
| DeepNote API response time | < 200ms for all endpoints |
| Bidirectional sync latency | < 5min (DeepBrain cognitive cycle interval) |
...
3. Feature works with DeepBrain running
4. Feature degrades gracefully with DeepBrain offline
