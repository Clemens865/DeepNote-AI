    ...
    <ArtifactWrapper title={data.title || 'Timeline'} jsonData={data}>
      <div className="p-4 bg-white dark:bg-zinc-900 overflow-x-auto">
        <div className="relative min-w-max">
          {/* Horizontal line */}
          <div className="absolute top-4 left-0 right-0 h-0.5 bg-black/[0.06] dark:bg-white/[0.06]" />

          <div className="flex gap-0">
            {sorted.map((event, i) => (
              <div key={i} className="relative flex flex-col items-center" style={{ minWidth: 140 }}>
                {/* Dot */}
                <div className="relative z-10 w-3 h-3 rounded-full bg-indigo-500 ring-4 ring-white dark:ring-zinc-900 flex-shrink-0" />
                ...
                <div className="mt-3 text-center px-2">
                  ...
                  <p className="text-xs font-medium text-zinc-800 dark:text-zinc-200 mt-0.5 leading-snug">
                    {event.label}
                  </p>
                  {event.description && (
                    <p className="text-[10px] text-zinc-500 dark:text-zinc-400 mt-0.5 leading-snug max-w-[120px]">
                      {event.description}
                    </p>
                  )}
                </div>
