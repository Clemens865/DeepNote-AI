...
function CompetitiveAnalysisContent({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-5">
      {analysisTitle && <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{analysisTitle}</h3>}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800">
                <th className="px-3 py-2 text-left font-semibold text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 sticky left-0 bg-slate-50 dark:bg-slate-800">
                  Feature
                </th>
                {competitors.map((comp, i) => (
                  <th key={i} className="px-3 py-2 text-center font-semibold text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
                    {comp.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {features.map((feature, fi) => (
                <tr key={fi} className="border-b border-slate-100 dark:border-slate-700/50 last:border-b-0">
                  <td className="px-3 py-2 font-medium text-slate-700 dark:text-slate-300 sticky left-0 bg-white dark:bg-slate-900">
                    {feature.name}
                  </td>
              ...
              <tr className="bg-slate-50 dark:bg-slate-800/50">
                <td className="px-3 py-2 font-bold text-slate-800 dark:text-slate-200 sticky left-0 bg-slate-50 dark:bg-slate-800/50">
                  Overall
                </td>
                {competitors.map((comp, ci) => {
                  ...
                  return (
                    <td key={ci} className="px-3 py-2 text-center">
                      <span className="text-sm font-bold text-slate-800 dark:text-slate-100">{avg.toFixed(1)}</span>
                    </td>
                  )
                })}
              </tr>
        ...
        <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${Math.min(competitors.length, 3)}, 1fr)` }}>
          {competitors.map((comp, i) => (
            <div key={i} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
              <h5 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">{comp.name}</h5>
                ...
                <div className="mb-2">
                  ...
                  <ul className="space-y-0.5">
                    {comp.strengths.map((s, si) => (
                      <li key={si} className="text-xs text-slate-600 dark:text-slate-400 flex gap-1">
                ...
                <div>
                  ...
                  <ul className="space-y-0.5">
                    {comp.weaknesses.map((w, wi) => (
                      <li key={wi} className="text-xs text-slate-600 dark:text-slate-400 flex gap-1">
