    ...
    <div className="space-y-6">
      ...
      {abstract && (
        <div className="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-xl p-5">
          ...
          <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{abstract}</p>
        </div>
      )}
      ...
      {tableOfContents.length > 0 && (
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          >
            <ListOrdered size={14} className="text-slate-500 dark:text-slate-400" />
            <span className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide flex-1">Table of Contents</span>
            {tocOpen ? <ChevronDown size={14} className="text-slate-400" /> : <ChevronRight size={14} className="text-slate-400" />}
          </button>
          {tocOpen && (
            <div className="px-4 pb-3 space-y-1">
              {tableOfContents.map((entry, i) => (
                <div key={i} className="flex items-center gap-2 text-sm">
                  <span className="text-slate-400 dark:text-slate-500 font-mono text-xs w-6 text-right">{entry.number}.</span>
                  <span className="text-slate-700 dark:text-slate-300">{entry.title}</span>
                </div>
              ))}
              <div className="flex items-center gap-2 text-sm">
                <span className="text-slate-400 dark:text-slate-500 font-mono text-xs w-6 text-right" />
                <span className="text-slate-500 dark:text-slate-400 italic">Conclusion</span>
              </div>
              {references.length > 0 && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-slate-400 dark:text-slate-500 font-mono text-xs w-6 text-right" />
                  <span className="text-slate-500 dark:text-slate-400 italic">References</span>
                </div>
              )}
            </div>
          )}
      ...
      {sections.map((section, i) => (
        <div key={i} className="space-y-3">
          ...
          {/* Section image */}
          {section.imagePath && (
            <figure className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
              <img
              ...
              {section.imageCaption && (
                <figcaption className="bg-slate-50 dark:bg-slate-800 px-4 py-2 text-xs text-slate-500 dark:text-slate-400 italic">
                  {section.imageCaption}
                </figcaption>
              )}
            </figure>
      ...
      {conclusion && (
        <div className="space-y-3">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Conclusion</h2>
          <div className="prose-container">
            <ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
              {conclusion}
            </ReactMarkdown>
      ...
      {references.length > 0 && (
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
          <div className="flex items-center gap-2 mb-3">
            <Quote size={14} className="text-slate-500 dark:text-slate-400" />
            <h3 className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">References</h3>
          </div>
          <ol className="space-y-2">
            {references.map((ref) => (
              <li key={ref.number} className="flex gap-2 text-xs text-slate-600 dark:text-slate-400">
                <span className="text-slate-400 dark:text-slate-500 font-mono flex-shrink-0">[{ref.number}]</span>
